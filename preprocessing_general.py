...
"""preprocessing_general.py: Process Illumina sequencing reads from fastq files."""
__author__      = "Sarah Fahlberg"
...
'''
# input directory containing fastq files (make sure to unzip them)
input_directory = '/mnt/rdrive/promero2/General/Sarah/NGS_data/210806_BH2JNMDMXY/'
date = '210806'
merged_reads_directory = 'merged_reads/'
# seq_df should contain a column called 'modified codons' which has the nucleotide seq we want to match
seq_df = pd.read_csv('ARCHIVAL_2021-03-24_GB1_twist_designs_correct_2000.csv')
output_counts_file = 'counts_test.csv'
'''
...
def process_read_pair(fwd_all_data, rev_all_data):
    ...
    # merge reads
    sequence = [None]*270 # this portion will be unique to each protein library (lines 84-90)
    for i in range(270):
        if i < 151:
            base = rev_read[i]
